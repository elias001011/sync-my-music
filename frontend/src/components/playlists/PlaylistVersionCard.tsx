...
export function PlaylistVersionCard({ accounts, entries }: { accounts: Account[]; entries: Record<string, ProviderPlaylistsEntry> }) {
  const sources = useMemo(() => accounts.filter((a) => a.state === 'connected' && a.enabled !== false && a.capabilities?.playlist_write), [accounts])
  ...
  async function loadVersions() {
    ...
    try { const rows = await api.getPlaylistVersions(activeProvider, playlist); setVersions(rows); setSelected(rows[0]?.captured_at ?? '') } catch (err) { setError(errorMessage(err)) } finally { setBusy('') }
  }
