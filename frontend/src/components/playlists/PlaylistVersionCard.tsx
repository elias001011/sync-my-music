...
export function PlaylistVersionCard({ accounts, entries }: { accounts: Account[]; entries: Record<string, ProviderPlaylistsEntry> }) {
  ...
  return (
    <Card className="p-4 sm:p-5">
      <div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-card bg-info-soft text-info"><LuHistory className="size-5" /></span><div><h2 className="text-[17px] font-bold text-text">{t('versions.title')}</h2><p className="mt-1 text-sm text-text-3">{t('versions.description')}</p></div></div>
      <div className="mt-4 grid gap-3 md:grid-cols-[200px_1fr_auto]">
        ...
        <select value={playlist} onChange={(e) => { setPlaylist(e.target.value); setVersions([]); setPlan(null) }} className="h-11 rounded-control border border-border-strong bg-field px-3 text-sm text-text"><option value="">{t('musify.choosePlaylist')}</option>{playlists.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
        <Button variant="secondary" loading={busy === 'load'} disabled={!playlist} onClick={() => void loadVersions()}>{t('versions.load')}</Button>
      </div>
      {versions.length > 0 && <div className="mt-3 flex flex-wrap items-end gap-3"><label className="min-w-64 flex-1 text-xs font-semibold text-text-2">{t('versions.snapshot')}<select value={selected} onChange={(e) => { setSelected(e.target.value); setPlan(null) }} className="mt-1.5 h-11 w-full rounded-control border border-border-strong bg-field px-3 text-sm text-text">{versions.map((version) => <option key={version.version_id ?? version.captured_at} value={version.version_id ?? version.captured_at}>{new Date(version.captured_at).toLocaleString(localeTag(locale))} · {t('versions.trackCount', { count: version.item_count })}</option>)}</select></label><Button loading={busy === 'preview'} onClick={() => void restore(false)}>{t('versions.preview')}</Button></div>}
      {!busy && playlist && versions.length === 0 && !error && <p className="mt-3 rounded-control bg-inset p-3 text-xs text-text-3">{t('versions.empty')}</p>}
      {plan && <div className="mt-3 flex flex-wrap items-center gap-3 rounded-control border border-warning bg-warning-soft p-3"><div className="min-w-0 flex-1"><strong className="text-sm text-text">{t('versions.plan', { additions: plan.additions, removals: plan.removals })}</strong><p className="text-xs text-text-3">{t('versions.membership', { count: plan.target_count, note: plan.order_note })}</p></div>{!plan.execute && <Button icon={<LuRotateCcw className="size-4" />} loading={busy === 'restore'} disabled={plan.additions === 0 && plan.removals === 0} onClick={() => void restore(true)}>{t('versions.apply')}</Button>}</div>}
