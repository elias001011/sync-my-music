...
| Transferência entre serviços | Implementado | Playlists; Musify pode ser origem manual. |
...
| Múltiplas contas no mesmo serviço | Fundação apenas | O schema aceita `service_accounts`, mas conectores, credenciais, jobs e UI ainda usam uma conta `:default`. |
| Spotify totalmente sem OAuth | Parcial | `sp_dc` já cobre escritas e playlists conhecidas; leituras gerais ainda dependem do cliente OAuth. |
...
- Criar perfis de credenciais isolados e IDs estáveis por conta.
...
- Garantir que recaps possam separar ou combinar contas sem duplicar eventos.
